        ...
        <div class="w-screen h-fit flex lg:mt-[70px] ">
            ...
            <div class="lg:flex lg:flex-row w-full ">
                <div class="lg:w-[25%] lg:h-screen lg:fixed ">
                    ...
                    <div class=" h-60 lg:h-full w-full bg-cover"
                ...
                <div class="lg:w-[75%] lg:grid-cols-2 lg:grid lg:ml-[25%]">
                    @foreach ($diet_days as $day)
                        ...
                        @if ($enrollment[0]->finished_day >= $i)
                            <div class="mb-3 mr-3 ml-3 p-2 m-3 border-2 border-cGreen rounded-3xl shadow-md relative z-0">
                                <div>
                                    ...
                                    <h2 class="text-center text-cGreen font-bold text-md">
                                        {{ $day->calories . ' Kcal Consumed' }} </h2>
                                    ...
                                    <div class="flex items-center justify-end">
                                        <button type="submit"
                                            class="w-40 h-fit py-2 px-5  rounded-full bg-cGreen text-white border-2 hover:bg-white hover:border-cGreen hover:text-cGreen duration-300 ease-out cursor-pointer text-center text-2xl">
                                            DONE
                                        </button>
                                    </div>
                                </div>
                                {{-- overlay --}}
                                <div
                                    class="absolute inset-0 flex flex-col justify-center items-center bg-cDarkGrey z-10 opacity-70 rounded-3xl">
                                </div>
                        ...
                        @elseif ($flag == 0 and $enrollment[0]->finished_day == 0)
                            ...
                            @endphp
                            <div
                                class="m-3 lg:mb-3 lg:mr-3 lg:ml-3 lg:mt-0 p-2 border-2 border-cGreen rounded-3xl shadow-md">
                                <h1 class="text-center text-cGreen font-bold text-2xl">{{ 'DAY ' . $i++ . ' :' }}</h1>
                        ...
                        @elseif ($flag == 0 and $today == $updated_at)
                            <div
                                class="flex flex-col justify-center m-3 lg:mb-3 lg:mr-3 lg:ml-3 p-2 border-2 border-cGreen rounded-3xl shadow-md">
                                <h1 class=" text-center  text-cGreen font-bold text-2xl">{{ 'DAY ' . $i++ . ' :' }}</h1>
                                {{-- Done --}}
                        ...
                        @else
                            <div
                                class="m-3 lg:mb-3 lg:mr-3 lg:ml-3 p-2 border-2 border-cGreen rounded-3xl shadow-md relative z-0">
                                <div>
                                    <h1 class="text-center text-cGreen font-bold text-2xl">{{ 'DAY ' . $i++ . ' :' }}</h1>
                                </div>
                                {{-- overlay --}}
